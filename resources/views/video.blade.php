...
<body style='margin : 0px; overflow: hidden;'>
  <a-scene device-orientation-permission-ui="enabled: false" vr-mode-ui="enabled: false" markerhandler embedded arjs='sourceType: webcam; debugUIEnabled: false;' arjs embedded renderer="logarithmicDepthBuffer: true;" gesture-detector id="scene">
    ...
    <a-assets>
      <video id="{{ $sparepart->id }}" src="{{ url('/upload/video/'.$sparepart->fileVideo) }}" preload="auto" response-type="arraybuffer" autoplay loop crossorigin webkit-playsinline playsinline></video>
    </a-assets>
    ...
    <a-marker type='pattern' url='{{ asset('../pattern-ARSparepart.patt') }}' markerhandler smooth="true" smoothCount="5" smoothTolerance="0.01" smoothThreshold="2">
      <a-text value="Video: {{ $sparepart->namaSP }}" color="yellow" rotation="-90 0 0" position="0 1.5 -1.2" align="center"></a-text>
      <a-video src="#{{ $sparepart->id }}" scale="1 1 1" position="0 0.4 0" rotation="-90 0 0" width="3" height="1.5"></a-video>
    </a-marker>
